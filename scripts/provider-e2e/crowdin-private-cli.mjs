  ...
  const remoteBase = async () =>
    (
      await command("gh", [
        ...
        "--method",
        "GET",
        `repos/${repository}/git/ref/heads/aidan/crowdin-private-recording-base-20260911`,
        "--jq",
        ".object.sha",
      ])
    ).trim();
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  runPrivatePreparation({
    verifyLive: process.argv[2] === "--verify-live",
    outputDir:
      process.argv.length === 4 && ["--prepare", "--verify-live"].includes(process.argv[2])
        ? process.argv[3]
        : undefined,
  }).catch((error) => {
