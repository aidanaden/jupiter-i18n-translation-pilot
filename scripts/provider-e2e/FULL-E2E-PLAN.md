...
Remaining exact decisions: permission to disable only non-production builds for the existing pilot, then final exact merge and deployment approval after checks. The first change prevents test pushes from uploading versions to the existing pilot. It must not change production builds from `main` or either scheduler. Provider context/glossary support must be verified before any translation-quality comparison. The JSON route previously omitted context during generation; restoring comments afterward does not remove that limit.
