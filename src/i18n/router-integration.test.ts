...
describe("Lingui router hydration", () => {
  it("keeps the pinned source copy exact and protected values unchanged in the pseudolocale", () => {
    ...
    expect(
      englishI18n._({ ...ONBOARD_BUY_DESCRIPTION_MESSAGE, values: ONBOARD_PROTECTED_VALUES }),
    ).toBe(
    ...
    expect(
      englishI18n._({ ...ONBOARD_CEX_DESCRIPTION_MESSAGE, values: ONBOARD_PROTECTED_VALUES }),
    ).toBe(
  ...
  it("renders Chinese on the first client render after hydrating the server locale", async () => {
    ...
    const clientRouter = createHydrationRouter(clientI18n, false, (locale) =>
...
function TestDocument() {
  ...
  return createElement(
    "html",
    { lang: i18n.locale },
    createElement("body", null, createElement(Outlet)),
  );
}
